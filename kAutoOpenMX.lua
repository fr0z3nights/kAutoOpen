...
kAutoOpen80:Register('BAG_UPDATE_DELAYED', function(bag)
	...
	for bag = 0, 4 do
		for slot = 1, C_Container.GetContainerNumSlots(bag) do
			...
			--	Dalaran
			if	GetMinimapZoneText() == "Dalaran" and id and autoOpenItems80[id] then
			...
			--	Stonelight Rest
			if	GetMinimapZoneText() == "Stonelight Rest" and id and autoOpenItems80[id] then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34FMX " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Stonelight Rest
			if	GetMinimapZoneText() == "Krasus' Landing" and id and autoOpenItems80[id] then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34F07. " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Infinite Bazaar
			if	GetMinimapZoneText() == "Infinite Bazaar" and id and autoOpenItems80[id] then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34F07. " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
