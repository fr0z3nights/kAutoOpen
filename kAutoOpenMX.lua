...
local autoOpenItems80 = {
	...
	--	Level 80 caches cause chat spam on low levels, 
			[264675] = true,	--	Cache from the Infinite's Armory					Looted	251210				
		--	Anniversary Event
			[245589] = true,	--	Hellcaller Chest									Looted 250701				Diablo Anniversary, Loot Issues, Currency Lockout?
...
kAutoOpen80:Register('BAG_UPDATE_DELAYED', function(bag)
	...
	for bag = 0, 4 do
		for slot = 1, C_Container.GetContainerNumSlots(bag) do
			...
			if	GetMinimapZoneText() == "Wizard's Sanctum" and id and autoOpenItems80[id] and canOpen then
			...
			--	Horde Portal Room
			if	GetMinimapZoneText() == "Pathfinder's Den" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34FMX " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Dalaran Portal Room
			if	GetMinimapZoneText() == "Chamber of the Guardian" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34FMX " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Dornogal
			if	GetMinimapZoneText() == "Foundation Hall" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34FMX " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Caverns of Time
			if	GetMinimapZoneText() == "Caverns of Time" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34FMX " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Dalaran
			if	GetMinimapZoneText() == "Dalaran" and id and autoOpenItems80[id] and canOpen then
			...
			--	Stonelight Rest
			if	GetMinimapZoneText() == "Stonelight Rest" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34FMX " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Stonelight Rest
			if	GetMinimapZoneText() == "Krasus' Landing" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34F07. " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
			...
			--	Infinite Bazaar
			if	GetMinimapZoneText() == "Infinite Bazaar" and id and autoOpenItems80[id] and canOpen then
				DEFAULT_CHAT_FRAME:AddMessage("|cFFFFD700!AutoOpen |cFF4FE34F07. " .. C_Container.GetContainerItemLink(bag, slot))
				C_Container.UseContainerItem(bag, slot)
				kAutoOpen_LastOpenTime = GetTime()
				return
